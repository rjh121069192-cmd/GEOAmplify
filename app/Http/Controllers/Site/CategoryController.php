...
class CategoryController extends Controller
{
    public function show(string $slug): View
    {
        ...
        $perPage = max(1, min(200, (int) ($map['per_page'] ?? config('geoamplify.items_per_page', 12))));
        $siteTitle = (string) ($map['site_name'] ?? config('geoamplify.site_name', config('app.name')));
        $siteDescription = (string) ($map['site_description'] ?? config('geoamplify.site_description', ''));
        $siteKeywords = (string) ($map['site_keywords'] ?? config('geoamplify.site_keywords', ''));
