 ...
 *
 * 设计目标：
 * 1. 仅从 config('geoamplify.api_key_crypto_roots') 派生加解密密钥（配置层绑定 APP_KEY），禁止应用代码直接 env()；
...
class ApiKeyCrypto
{
    ...
    public function encrypt(string $apiKey): string
    {
        ...
        if (! is_string($primaryKey) || $primaryKey === '') {
            throw new RuntimeException('APP_KEY 未配置或 config(geoamplify.api_key_crypto_roots) 为空，无法加密 API Key');
        }
    ...
    /**
     * 从配置解析加解密密钥（与 config/geoamplify.php 中 api_key_crypto_roots 顺序一致，便于密钥轮换时兼容旧密文）。
     *
     * @return list<string>
     */
    ...
    {
        $keys = [];
        $roots = config('geoamplify.api_key_crypto_roots', []);
        if (! is_array($roots)) {
            $roots = [];
        }
