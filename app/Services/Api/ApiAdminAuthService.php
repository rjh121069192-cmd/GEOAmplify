...
use App\Support\GeoAmplify\AdminLoginLockService;
...
class ApiAdminAuthService
{
    ...
    public function login(string $username, string $password, string $ipAddress = '', string $userAgent = ''): array
    {
        ...
        $maxAttempts = max(1, (int) config('geoamplify.api_login_rate_limit_attempts', 10));
        $decaySeconds = max(1, (int) config('geoamplify.api_login_rate_limit_decay_seconds', 60));
