...
namespace App\Services\GeoAmplify;
...
class KnowledgeChunkSyncService
{
    ...
    private function planChunks(int $knowledgeBaseId, string $content): array
    {
        ...
        if (! $this->canAttemptSemanticChunking($blocks)) {
            Log::info('geoamplify.knowledge_semantic_chunking_skipped', [
                'knowledge_base_id' => $knowledgeBaseId,
    ...
    private function buildSemanticChunks(int $knowledgeBaseId, array $blocks): array
    {
        ...
        foreach ($models as $model) {
            ...
            if ($providerUrl === '' || $apiKey === '' || $modelId === '') {
                Log::info('geoamplify.knowledge_semantic_chunking_model_skipped', [
                    'knowledge_base_id' => $knowledgeBaseId,
            ...
            try {
                ...
                if ($chunks === []) {
                    Log::info('geoamplify.knowledge_semantic_chunking_invalid_response', [
                        'knowledge_base_id' => $knowledgeBaseId,
                        'semantic_model_id' => (int) $model->id,
                        'model_identifier' => $modelId,
                ...
                Log::info('geoamplify.knowledge_semantic_chunking_failed', [
                    'knowledge_base_id' => $knowledgeBaseId,
                    'semantic_model_id' => (int) $model->id,
                    'model_identifier' => $modelId,
                    'provider_url' => $providerUrl,
    ...
    {
        return max(1, (int) config('geoamplify.semantic_chunking_max_chars', self::SEMANTIC_CHUNKING_MAX_PROMPT_CHARS));
    }
     ...
     *
     * 观测：开启 {@see config('geoamplify.debug_knowledge_query_embedding')} 时写入 `geoamplify.knowledge_query_embedding` 日志。
     */
    public function generateQueryVectorLiteral(string $query): string
    {
        $debug = (bool) config('geoamplify.debug_knowledge_query_embedding', false);
        $query = trim($query);
        if ($query === '') {
            if ($debug) {
                Log::info('geoamplify.knowledge_query_embedding', ['outcome' => 'skip_empty_query']);
            }
        ...
        if (! $this->canStoreEmbeddingVector()) {
            if ($debug) {
                Log::info('geoamplify.knowledge_query_embedding', ['outcome' => 'skip_no_pgvector_storage']);
            }
        ...
        if ($debug) {
            Log::info('geoamplify.knowledge_query_embedding', [
    ...
    public function generateQueryEmbeddingVector(string $query): array
    {
        ...
        try {
            ...
            Log::info('geoamplify.knowledge_query_embedding_failed', [
    ...
    private function generateEmbeddingsForChunks(array $chunks, ?array $embeddingMetadata, bool $requireRealEmbedding = false, ?string $documentTitle = null): array
    {
        ...
        try {
            ...
            $message = OpenAiRuntimeProvider::normalizeApiException($exception, (string) ($embeddingMetadata['api_url'] ?? ''));
            Log::info('geoamplify.knowledge_embedding_failed', [
                'embedding_model_id' => (int) ($embeddingMetadata['model_id'] ?? 0),
                'model_identifier' => (string) ($embeddingMetadata['model_name'] ?? ''),
                'message' => $message,
            ]);
    ...
    {
        return max(1, min(64, (int) config('geoamplify.embedding_batch_size', 1)));
    }
