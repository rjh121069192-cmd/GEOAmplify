...
class AdminUpdateMetadataService
{
    ...
    {
        return trim((string) config('geoamplify.app_version', '1.2.0'));
    }

    public function metadataUrl(): string
    {
        return trim((string) config('geoamplify.update_metadata_url', ''));
    }

    public function isEnabled(): bool
    {
        return (bool) config('geoamplify.update_check_enabled', true) && $this->metadataUrl() !== '';
    }
    ...
    private function fetchRemoteMetadata(string $url): array
    {
        ...
        $ttl = max(60, (int) config('geoamplify.update_metadata_cache_ttl_seconds', 86400));
